...
import type { TroopDeploymentEvent, DeploymentDetectionConfig } from '../../utils/troopDeploymentDetector';
...
const TroopDeploymentPanel: React.FC<TroopDeploymentPanelProps> = ({ state, actions }) => {
    ...
    const [selectedDeployment, setSelectedDeployment] = useState<TroopDeploymentEvent | null>(null);

    const { deploymentEvents, deploymentStats, isDetectionEnabled, config } = state;
        ...
        <div className="space-y-4">
            ...
            {selectedDeployment && (
                <div className="bg-black/30 rounded-xl p-4 border border-cr-purple/20">
                    ...
                    {selectedDeployment.annotatedFrame && (
                        <div className="mb-4">
                            <img
                                src={`data:image/jpeg;base64,${selectedDeployment.annotatedFrame}`}
                                alt="Deployment frame"
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                        <div>
                            ...
                            <span className="ml-2 text-white">
                                ({Math.round(selectedDeployment.centerX)}, {Math.round(selectedDeployment.centerY)})
                            </span>
                        </div>
                        <div>
                            <span className="text-white/60">Objects:</span>
                            <span className="ml-2 text-white">{selectedDeployment.detectionCount}</span>
                        </div>
                        <div>
                            <span className="text-white/60">Duration:</span>
                            <span className="ml-2 text-white">{selectedDeployment.duration}ms</span>
                        </div>
                        <div>
                            <span className="text-white/60">Confidence:</span>
                            <span className="ml-2 text-white">{(selectedDeployment.confidence * 100).toFixed(1)}%</span>
                        </div>
                        <div className="col-span-2">
                            <span className="text-white/60">Troops:</span>
                            <span className="ml-2 text-white">
                                {selectedDeployment.troopTypes.map(type => getTroopEmoji(type)).join(' ')}
                                {selectedDeployment.troopTypes.join(', ')}
                            </span>
                        </div>
                        <div className="col-span-2">
            ...
            {/* Main panel - only show if no deployment selected */}
            {!selectedDeployment && (
                <>
                    ...
                    {deploymentEvents.length > 0 && (
                        <div className="bg-black/30 rounded-xl p-4 border border-cr-purple/20">
                            <h4 className="text-white font-medium mb-3 flex items-center gap-2">
                                ...
                                All Deployments ({deploymentEvents.length})
                            </h4>
                                ...
                                <AnimatePresence>
                                    {deploymentEvents.map((deployment) => (
                                        <motion.button
                                            ...
                                            className="w-full bg-black/20 rounded-lg p-3 border border-white/10 hover:border-cr-purple/30 hover:bg-black/30 transition-all cursor-pointer"
                                        >
                    ...
                    {deploymentEvents.length === 0 && isDetectionEnabled && (
