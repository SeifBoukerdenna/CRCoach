// royal_trainer_client/src/components/GameMechanicsTester.tsx
...
const GameMechanicsTester: React.FC = () => {
    ...
    const handlePlayCard = (card: Card) => {
        ...
        console.log(`🔍 Can afford? ${elixir.current >= card.cost ? 'YES' : 'NO'}`);

        const canPlay = elixir.spendElixir(card.cost);
        console.log(`💰 spendElixir returned: ${canPlay}`);
        ...
        <div className="max-w-6xl mx-auto p-6 space-y-8">
            ...
            >
                ...
                <p className="text-white/60 mb-2">
                    Test advanced elixir regeneration and card cycling hooks
                </p>
                <div className="bg-blue-900/30 border border-blue-500/30 rounded-lg p-3 max-w-4xl mx-auto">
                    <p className="text-sm text-blue-200">
                        ...
                        🃏 <strong>Cards:</strong> 4 cards in hand + 4 in cycle queue. Play any hand card → goes to back of 8-card cycle → next queued card enters hand
                    </p>
                </div>
            </motion.div>
                ...
                >
                    ...
                    <div className="space-y-3">
                        <div className="text-sm text-white/70">
                            <p>Next Card: <span className="text-yellow-400 font-semibold">{cardCycle.nextCard.name}</span></p>
                            <p>Cost: <span className="text-purple-400">{cardCycle.nextCard.cost}</span> elixir</p>
                            <p className="text-xs text-white/50 mt-1">🎯 Play any card from hand → cycles to back → next enters hand</p>
                        </div>

                        <div className="flex gap-2">
                            <button
                                onClick={cardCycle.shuffleDeck}
                                className="flex-1 flex items-center justify-center gap-2 px-3 py-2 bg-orange-600 hover:bg-orange-700 rounded-lg text-sm font-semibold transition-all"
                            >
                                <Shuffle className="w-4 h-4" />
                                Shuffle
                            </button>
                            <button
                                onClick={cardCycle.resetCycle}
                                className="flex items-center justify-center gap-2 px-3 py-2 bg-gray-600 hover:bg-gray-700 rounded-lg text-sm font-semibold transition-all"
                            >
                                <RotateCcw className="w-4 h-4" />
                                Reset
                ...
                >
                    ...
                    <div className="space-y-2 text-sm">
                        ...
                        <div className="flex justify-between">
                            ...
                            <span className="text-yellow-400 font-bold">{cardCycle.hand.length}/4</span>
                        </div>
                    </div>
                </motion.div>
            </div>
            ...
            >
                <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="text-sm text-white/60">
                        Next: <span className="text-yellow-400 font-semibold">{cardCycle.nextCard.name}</span>
                            ...
                            >
                                ...
                                <div className="absolute -bottom-8 left-1/2 transform -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none">
                                    <div className="bg-black/90 text-white text-xs px-2 py-1 rounded whitespace-nowrap">
                                        {card.name} ({card.cost} elixir, Lv.{card.level}) - Position #{index + 1}
                                    </div>
                                </div>
                            </motion.div>
                        );
            ...
            >
                ...
                <div className="grid grid-cols-4 gap-4">
                    {cardCycle.cycleQueue.map((card, index) => (
                        <motion.div
                            key={`queue-${card.id}-${cardCycle.getCardPosition(card.id)}`} // Force re-render on position change
                            className="relative"
                            initial={{ opacity: 0, y: 20 }}
                            animate={{ opacity: 1, y: 0 }}
