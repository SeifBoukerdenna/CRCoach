...
import { formatQualityLabel, formatResolution, getQualityColorClass } from "../../utils/formatters";

interface MetricsProps {
    /** Video resolution (format: "width×height") */
    resolution: string;
    /** Frames per second with suffix */
    fps: string;
    /** Round-trip time with suffix */
    rtt: string;
    /** Stream quality level */
    quality?: StreamQuality;
    /** Whether high latency is detected */
...
/**
 * Metrics component displays broadcast performance statistics
 */
export const Metrics: React.FC<MetricsProps> = ({
    ...
    // Icon components for each quality level
    const QualityIcon = () => {
        switch (quality) {
            case "low":
                return (
    ...
    return (
        <div className={`metrics ${className}`} data-lag={isHighLatency ? "true" : "false"}>
            {/* Resolution display */}
            ...
            <span className={`metric-item quality ${getQualityColorClass(quality)}`}>
                <QualityIcon /> {formatQualityLabel(quality)}
            </span>
        </div>
    );
};
